...
/**
 * Flow-11 MSA Dynamic VM Upstream Orchestration Integration Test (JDK 1.8 Compatible)
 * - Target: Verify runtime destination determination and MSA chaining via VM Upstream
 * - Tool: OkHttp3, Commons-IO (Test Scope)
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
public class SpELVMModifyTestCase {
    private static final Logger log = LoggerFactory.getLogger(SpELVMModifyTestCase.class);
    ...
    void setUp() throws Exception {
        ...
        // 2. Generate mock received byte data (Java 8 compatible)
    ...
    @Test
    @DisplayName("[EventBuilder] Does it extract TrxID from raw bytes via SpEL and layout?")
    ...
    @Test
    @DisplayName("[Lazy Parsing] Verify layout-based byte partitioning (Map projection)")
    void testLayoutLazyParsing() throws Exception {
        ...
        assertEquals("TRX-9999-8888-7777-6666-5555-444", parsedBody.get("GLOBAL_TRX_ID"));
    }

    @Test
    @DisplayName("[Flow Simulation] Mock test of FIXED_VALUE_MODIFIER filter behavior")
    ...
    @Test
    @DisplayName("[Real HTTP] Verify dynamic sub-flow (XML) assembly and invocation of DynamicLocalUpstream")
    void testDynamicVmUpstreamToXmlSubFlow() throws Exception {
        ...
        mockTargetServer.createContext("/", ex -> {
            // JDK 1.8 compatibility: Use IOUtils instead of readAllBytes()
        ...
        try {
            URI targetUri = URI.create("http://127.0.0.1:51006/");
            assumeTrue(isServerUp(targetUri), "Engine is not up. Skipping test.");
    ...
    @Test
    @DisplayName("[Real HTTP] Verify Fail-Fast blocking of VALIDATOR guardrail filter")
    ...
    @Test
    @DisplayName("[Real HTTP] Verify dynamic sub-flow (JSON) assembly and invocation of DynamicLocalUpstream")
    void testDynamicVmUpstreamToJsonSubFlow() throws Exception {
        com.sun.net.httpserver.HttpServer mockTargetServer = com.sun.net.httpserver.HttpServer.create(new java.net.InetSocketAddress(51101), 0);
        mockTargetServer.createContext("/", ex -> {
            byte[] body = IOUtils.toByteArray(ex.getRequestBody());
            ex.sendResponseHeaders(200, body.length);
    ...
    private boolean isServerUp(URI targetUri) {
        try (java.net.Socket socket = new java.net.Socket()) {
            socket.connect(new java.net.InetSocketAddress(targetUri.getHost(), targetUri.getPort()), 1000);
            return true;
